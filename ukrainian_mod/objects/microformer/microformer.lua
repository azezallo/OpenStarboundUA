...
function onInteraction(args)
  if not (storage.depleted or storage.addTimer) then
    ...
    if success then
      ...
      return { "ShowPopup", { title = "Активація не вдалася!", message = string.format("^red;Мікроформер не вдалося активувати: %s.", reason), sound = "/sfx/interface/nav_insufficient_fuel.ogg"} }
    end
  end
end
...
function canActivate(newRegionSize)
  if not world.terrestrial() then
    return false, "Має бути розміщений на планеті"
  end
  ...
  for k, pos in pairs(activeTerraformers) do
    if k ~= storage.uuid and world.magnitude(self.terraformPosition, pos) < checkRadius then
      return false, "Занадто близько до іншого активного тераформера"

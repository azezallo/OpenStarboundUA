...
function spawnObject(spawnConfig)
  ...
  if clueConfig then
    ...
    storage.questStorage[spawnConfig.questId].dialog[uniqueId] = RepTags(clueConfig.dialog, tags)
    if clueConfig.message then
      for _,playerId in ipairs(storage.questStorage[spawnConfig.questId].players) do
        ...
        self.outbox:sendMessage(playerId, spawnConfig.questId.."setCompleteMessage", RepTags(clueConfig.message, tags))
      end
    end
  end

...
function build(directory, config, parameters, level, seed)
  ...
  if config.primaryAbility then
    config.tooltipFields.primaryAbilityTitleLabel = "Основне:"
    config.tooltipFields.primaryAbilityLabel = config.primaryAbility.name or "Невідомо"
  end
  if config.altAbility and config.altAbility.name then
  ...
  if config.tooltipFields.levelLabel then
  ...
  config.price = (config.price or 0) * root.evalFunction("itemLevelPriceMultiplier", configParameter("level", 1))
	...
	if config.rarity == "uncommon" or config.rarity == "Uncommon" then config.tooltipFields.rarityLabel = "Незвичайний" end

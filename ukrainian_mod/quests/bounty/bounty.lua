...
require "/scripts/ReworkFunc.lua"
...
function init()
  ...
  self.defaultSkipMessages = {
    "Тобі вдалося розібратися без підказок? Чудова робота!"
  }
...
function questInvolvesWorld()
  ...
  if locationsParameter then
    local locationWorlds = util.map(util.tableValues(locationsParameter.locations), function(location)
        ...
        return RepTags(location.worldId or quest.worldId() or "", tags)
      end)
...
function questComplete()
  ...
  if quest.questId() == quests[#quests].questId then
    ...
    modifyQuestEvents("Виконано", rewards.money, rewards.rank, rewards.credits)
    ...
    text = RepTags(util.randomFromList(text), tags)
...
function questFail(abandoned)
  ...
  modifyQuestEvents("Провалено", 0, 0, 0)
...
function setText()
  ...
  local title
  if self.bountyType == "major" then
    --title = sb.replaceTags("^yellow; ^orange;Винагорода: ^green;<bounty.name>", tags)
    title = RepTags("^yellow; ^orange;Винагорода: ^green;<bounty.name>", tags)
  else
    --title = sb.replaceTags("^orange;Винагорода: ^green;<bounty.name>", tags)
    title = RepTags("^orange;Винагорода: ^green;<bounty.name>", tags)
  end
  ...
  for i, q in pairs(quest.questArcDescriptor().quests) do
    if i > 1 then -- skip the first quest, it's fake
      ...
      if textCons then
        ...
        textCons = string.format("%s%s", textCons, RepTags(text, tags))
      else
        --textCons = sb.replaceTags(text, tags)
        textCons = RepTags(text, tags)
      end
      ...
      if q.questId == quest.questId() then
        if questConfig.generatedText.failureText then
          ...
          failureText = RepTags(failureText, tags)
...
function radioMessage(text, portraitType)
  ...
  message.text = RepTags(text, tags)
  return message
end

...
function setTitleScene(savedState)
  ...
  if savedState then
    table.insert(titleScene.options, {"Продовжити", "LOAD", savedState})
  end
...
function setScene(scene, withTransition)
  ...
  if scene.text and withTransition then
	...
	--Перезаписана функція
    local t = coroutine.create(function()
    local ratio = 0
    local rate = dt / (utf8.len(scene.text) / self.textRate)

    while ratio < 1.0 do
      ratio = math.min(1.0, ratio + rate)

      local i = math.ceil(ratio * utf8.len(scene.text))
	  if i < utf8.len(scene.text) then
        local outStr = utf8.sub(scene.text, 1, i) .. "^clear;" .. utf8.sub(scene.text, i + 1, utf8.len(scene.text))

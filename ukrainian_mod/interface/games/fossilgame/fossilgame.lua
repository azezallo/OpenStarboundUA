...
function toolUsesRemaining()
  local count = 0
  --відлік починається з 2, бо пензель не враховуємо
  for _,tool in pairs(self.tools) do
    if tool.uses > -1 then
...
function drawGui()
  ...
  gameCanvas:drawText("ПРОГРЕС", {position = {235, 183}, width = 88}, 12)
  gameCanvas:drawText("РОЗКОПОК", {position = {235, 171}, width = 88}, 12)

...
function updateToolTip(position)
  ...
  if button ~= nil then
    ...
    if string.len(button) > 11 and string.sub(button, 1, 12)  == "templateSlot" then
      self.toolTip = "Сканування вивіски в слоті автомата"
      blankTip = false
    elseif string.len(button) > 11 and string.sub(button, 1, 12) == "templateSave" then
      local templateIndex = "slot"..string.sub(button, -1)
      self.toolTip = "Збереження \""..storage.signName.."\" у слот "..string.sub(templateIndex,-1)
      blankTip=false
    elseif button == "pixelPress" then
      if self.currentCursorModifier == "pickPixel" then
        self.toolTip = "[ЛКМ] - вибір кольору"
      elseif self.currentCursorModifier == "fillPixel" then
        self.toolTip = "[ЛКМ] - заповнення"
      else
        self.toolTip = "[ЛКМ] - малювати, [ПКМ] - очистити"
      end
      blankTip = false
    elseif button == "undoPress" then
      if #self.undoStack > 0 then
        self.toolTip = "Скасування попередньої дії ("..(undoTips[self.undoStack[#self.undoStack][1]] or "ВІДСУТНІЙ")..")"
        blankTip = false
      end
    else
      --most come from the array in customeasel-data.lua
      self.toolTip = toolTips[button] or "*ВІДСУТНІЙ TOOLTIP*"
      blankTip = false
    end
  end
...
function assignInputString(stringType, input)
  ...
  if isInvalid then
    self.thrownError = {300, "ПОМИЛКА! НЕВІРНИЙ АРГУМЕНТ", "ДИВІТЬСЯ КРИТЕРІЇ"}
  end
end
...
function outputSign()
  ...
  if not outputSuccess then
    self.thrownError = {500, "   ПОМИЛКА!", "     БУДЬ ЛАСКА, ОЧИСТІТЬ ВИХІД"}
  end
  --sb.logInfo("---------------------------------")
end

...
function BountyGenerator:processSteps(steps, bounty, planetPool)
  ...
  for i,step in pairs(steps) do
    ...
    for k,locationConfig in pairs(locations[step.questId]) do
      ...
      local worldId = locationConfig.worldId and RepTags(locationConfig.worldId, worldTags)
            ...
            for _, override in ipairs(overrides) do
              for k,v in pairs(override.behavior.parameters or {}) do
                ...
                if type(v) == "string" then
                  ...
                  override.behavior.parameters[k] = RepTags(v, tags)
                end
              end
            end
          end
  ...
  for _,step in pairs(steps) do
    ...
    if text then
      if text.completeMessage then
        ...
        text.completeMessage = RepTags(text.completeMessage, tags)
      end
      if text.skipMessage then
        --text.skipMessage = sb.replaceTags(text.skipMessage, tags)
        text.skipMessage = RepTags(text.skipMessage, tags)
      end
    end
  end

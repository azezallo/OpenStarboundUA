...
-- Функції для сутностей, які можна захопити за допомогою капсули
capturable = {}
...
-- Від'єднати цього улюбленця від капсули до наступного «зцілення» капсули.
...
-- Прив'язати іншого монстра до капсули цього монстра.
...
function capturable.captureStatus()
  ...
  -- Обчислити деякі штучні статистики для відображення в інвентарі, поруч з
  -- слотом для улюбленця:
...
function capturable.attemptCapture(podOwner)
  -- Спробувати захопити монстра. У разі успіху монстр гине і
  -- повертається конфігурація улюбленця.
...
function capturable.attemptRelocate(sourceEntity)
  if config.getParameter("relocatable", false) and not capturable.confirmRelocate then
    --Точка, до якої монстр масштабуватиметься
    local scaleOffsetPart = config.getParameter("scaleOffsetPart", "body")
    local attachPoint = vec2.div(animator.partPoint(scaleOffsetPart, "offset") or {0, 0}, 2) -- ділимо на два, бо partPoint додає зміщення до зміщення
    local petInfo = {

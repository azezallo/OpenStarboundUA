...
function speciesDialog(dialog, targetId)
  ...
  if dialog.generic and not dialog.neki then
...
function sayToEntity(args, board)
  ...
  context().say(RepTags(dialog, tags), tags, options)
  return true
end
...
function say(args, board)
  ...
  if portrait == nil then
    ...
    context().say(RepTags(args.content, args.tags), args.tags, options)
  else
    --УКР заміна
	--context().sayPortrait(args.content, portrait, args.tags, options)
    context().sayPortrait(RepTags(args.content, args.tags), portrait, args.tags, options)
  end
...
function sayNext(args, board)
  ...
  if portrait == nil then
    ...
    context().say(RepTags(self.currentDialog[1], args.tags), args.tags, options)
  else
    ...
    context().sayPortrait(RepTags(self.currentDialog[1], args.tags), portrait, args.tags, options)
  end
